...
CREATE TABLE IF NOT EXISTS agent_configurations (
  ...
  agent_personality VARCHAR DEFAULT 'professional' CHECK (
    agent_personality IN ('professional', 'friendly', 'technical')
  ),
...
CREATE POLICY "Users can view their own agents" ON agents FOR
SELECT USING (
    client_id = auth.uid()
    OR EXISTS (
      SELECT 1
...
CREATE POLICY "Users can insert their own agents" ON agents FOR
INSERT WITH CHECK (client_id = auth.uid());
CREATE POLICY "Users can update their own agents" ON agents FOR
UPDATE USING (
    client_id = auth.uid()
    OR EXISTS (
      SELECT 1
...
CREATE POLICY "Users can view their own business knowledge" ON business_knowledge FOR
SELECT USING (
    client_id = auth.uid()
    OR EXISTS (
      SELECT 1
...
CREATE POLICY "Users can insert their own business knowledge" ON business_knowledge FOR
INSERT WITH CHECK (client_id = auth.uid());
CREATE POLICY "Users can update their own business knowledge" ON business_knowledge FOR
UPDATE USING (client_id = auth.uid());
CREATE POLICY "Users can delete their own business knowledge" ON business_knowledge FOR DELETE USING (client_id = auth.uid());
-- RLS Policies for call_logs table
CREATE POLICY "Users can view their own call logs" ON call_logs FOR
SELECT USING (
    client_id = auth.uid()
    OR EXISTS (
      SELECT 1
...
CREATE POLICY "Service role can manage call logs" ON call_logs FOR ALL USING (auth.role() = 'service_role');
-- RLS Policies for agent_configurations table
CREATE POLICY "Users can view their own agent configurations" ON agent_configurations FOR
SELECT USING (
    client_id = auth.uid()
    OR EXISTS (
      SELECT 1
...
CREATE POLICY "Users can insert their own agent configurations" ON agent_configurations FOR
INSERT WITH CHECK (client_id = auth.uid());
CREATE POLICY "Users can update their own agent configurations" ON agent_configurations FOR
UPDATE USING (client_id = auth.uid());
-- RLS Policies for email_notifications table (admin only)
CREATE POLICY "Admins can view all email notifications" ON email_notifications FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM profiles
...
CREATE POLICY "Service role can manage email notifications" ON email_notifications FOR ALL USING (auth.role() = 'service_role');
...
CREATE TRIGGER update_business_knowledge_updated_at BEFORE
UPDATE ON business_knowledge FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_call_logs_updated_at BEFORE
UPDATE ON call_logs FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_agent_configurations_updated_at BEFORE
UPDATE ON agent_configurations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
-- Create function to get agent configuration with business knowledge
CREATE OR REPLACE FUNCTION get_agent_configuration_with_knowledge(agent_uuid UUID) RETURNS JSON AS $$
DECLARE result JSON;
BEGIN
