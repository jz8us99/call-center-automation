  ...
  try {
    ...
    const testConfig = {
      ...
      agentConfig: {
        ...
        basic_info_prompt: 'You are a test agent to verify dynamic LLM ID retrieval works.',
        call_scripts: {
    ...
    try {
      ...
      if (llms.length > 0) {
        ...
        llms.forEach((llm, index) => {
          console.log(`   ${index + 1}. ${llm.llm_id} (${llm.model_name || 'Unknown model'})`);
        });
      } else {
    ...
    console.log('   RETELL_LLM_ID:', hasLlmId ? '✅ Set (fallback available)' : '⚠️  Not set (will use dynamic)');
...
async function runTest() {
  ...
  if (result.success) {
    ...
    console.log('Agents will now automatically retrieve valid LLM IDs during deployment.');
  } else {
