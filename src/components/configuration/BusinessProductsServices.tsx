...
interface BusinessProductsServicesProps {
  user: User;
  serviceTypeCode: string;
  onProductsServicesUpdate: (hasProductsServices: boolean) => void;
}

export function BusinessProductsServices({
  user,
  serviceTypeCode,
  ...
  useEffect(() => {
    if (user && serviceTypeCode) {
      console.log('Loading initial data for:', serviceTypeCode, user.id);
      loadInitialData();
    }
  ...
  useEffect(() => {
    ...
    console.log(
      'Updating products/services status:',
      hasData,
      jobCategories.length,
      jobTypes.length
    );
    onProductsServicesUpdate(hasData);
  }, [jobCategories.length, jobTypes.length]);

  const loadInitialData = async () => {
    console.log('Starting loadInitialData');
    setLoading(true);
    try {
      await Promise.all([loadJobCategories(), loadJobTypes()]);
  ...
  const loadJobCategories = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Job categories data:', data);
        setJobCategories(data.categories || []);
      } else {
        console.error(
          'Job categories API error:',
          response.status,
          await response.text()
        );
      }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Configure your service categories and specific job types with
                pricing and duration for your {serviceTypeCode} business.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
          ...
          {jobCategories.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                Create your first service category to organize your offerings.
              </p>
              <Button
                onClick={() => {
                  resetCategoryForm();
      ...
      {showCategoryForm && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Create or update a service category for your business offerings.
            </CardDescription>
          </CardHeader>
        ...
        <CardContent>
          {jobTypes.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                Add specific services with pricing and duration information.
              </p>
              <Button
                onClick={() => {
                  resetJobTypeForm();
      ...
      {showJobTypeForm && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <Label htmlFor="job-display-order">Display Order</Label>
              <Input
                id="job-display-order"
                type="number"
                value={jobTypeForm.display_order}
                onChange={e =>
                  setJobTypeForm(prev => ({
                    ...prev,
                    display_order: parseInt(e.target.value) || 0,
            ...
            <div>
              ...
              <p className="text-sm text-blue-700">
                {jobCategories.length > 0 && jobTypes.length > 0
                  ? `${jobCategories.length} categories and ${jobTypes.length} job types configured. Your staff can now be assigned to specific services.`
                  : 'Configure at least one category and job type to enable staff assignment capabilities.'}
              </p>
            </div>
          </div>
        </CardContent>
